  ...
  implements VirtualScrollStrategy
{
  ...
  private updateContent() {
    ...
    const itemsDisplayed = Math.ceil(
      this.viewport.getViewportSize() / this.rowHeight,
    );
    ...
    const rowsToMove =
      Math.sign(rowsScrolled) * Math.floor(Math.abs(rowsScrolled));
    const adjustedRenderedOffset = Math.max(
      0,
      renderedOffset + rowsToMove * this.rowHeight,
    );
