...
// 1. Handle Flat Data => DataSource
// 2. Handle Tree Action => Stream the Data Souce
// 3. Handle TSV
//
// <B
const MAX_SAFE_INTEGER = 9007199254740991;

export class FlatTableDataSource<
  T,
  F,
  K = F,
...
> extends DataSource<F> {
  ...
  private readonly _renderData = new BehaviorSubject<F[]>([]);
  ...
  set data(data: T[]) {
    ...
    this._data.next(data);
    this._flattenedData.next(this._treeFlattener.flattenNodes(data));
    this._treeControl.dataNodes = this._flattenedData.value;
    // subscription, but that won't happen if it's inactive.
    if (!this._renderChangesSubscription) {
      this._filterData(this._treeFlattener.flattenNodes(data));
    }
  }
  ...
  set filter(filter: string) {
    ...
    // subscription, but that won't happen if it's inactive.
    if (!this._renderChangesSubscription) {
      this._filterData(this._flattenedData.value);
    }
  }
   ...
   */
  sortingDataAccessor: (data: F, sortHeaderId: string) => string | number = (
    data: F,
    sortHeaderId: string,
  ): string | number => {
   ...
   */
  sortData: (data: F[], sort: MatSort) => F[] = (
    data: F[],
    sort: MatSort,
  ): F[] => {
   ...
   */
  filterPredicate: (data: F, filter: string) => boolean = (
    data: F,
    filter: string,
  ): boolean => {
  ...
  constructor(
    ...
    super();
    const flattenData = this._treeFlattener.flattenNodes(initialData);
    this._treeControl.dataNodes = flattenData;
    this._flattenedData.next(flattenData);
   ...
   * @docs-private
   */
  override connect(collectionsViewer: CollectionViewer) {
    if (!this._renderChangesSubscription) {
      this._updateChangeSubscription();
    }
